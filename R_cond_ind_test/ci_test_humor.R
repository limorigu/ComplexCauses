...
ci_test <- function(dat) {
  df <- rbind(
    fread(paste0('../Python_Img_Humor/out/viz/Humicroedit/residuals_nested=TRUE_', dat, '.csv')), 
    fread(paste0('../Python_Img_Humor/out/viz/Humicroedit/residuals_nested=FALSE_', dat, '.csv'))
  )
  n <- nrow(df) / 2
...
fwrite(res, 'ci_test_results_humor.csv')
